...
fn parse_lib_rs(content: &str) -> Vec<LintEntry> {
    ...
    if let Some(start) = content.find("pub const LINT_METADATA: &[LintMetadata] = &[") {
        let after =
            &content[start + "pub const LINT_METADATA: &[LintMetadata] = &[".len()..];
...
fn generate_readme(entries: &[LintEntry]) -> String {
    ...
    md.push_str("<!-- Run `cargo run -p generate-lint-docs` from the workspace root to regenerate. -->\n\n");
    ...
    for cat in &category_order {
        if let Some(entries) = categorized.get(*cat) {
            ...
            for entry in entries {
                ...
                md.push_str(&format!(
                    "| [`{}`]({}) | `{}` | {} |\n",
                    entry.name_doc, doc_path, entry.level.to_lowercase(), entry.description
                ));
            }
            md.push('\n');
        }
...
fn main() {
    ...
    if !lib_path.exists() {
        eprintln!("Error: Cannot find soroban_cost_lints/src/lib.rs at {:?}", lib_path);
