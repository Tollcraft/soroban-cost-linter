...
The linter hooks into the Rust compiler's AST to catch specific Soroban anti-patterns. Four lints ship in `v0.1.1`:
...
*   **[`event_in_loop`](docs/lints/event_in_loop.md):** Detects `env.events().publish(...)` calls inside loop bodies, where each emission crosses the guest/host boundary and bloats the transaction footprint.
